...
public class InverseSubBytes {
	...
	private byte[][] subbedByteArray=new byte[4][4];
	
	public byte[][] invSubByteArray(byte[][] shiftedByteArray)
	{
		for(int i=0;i<4;i++)
			for(int j=0;j<4;j++)
			
				subbedByteArray[i][j]=invSubBytes(shiftedByteArray[i][j]);
			
		
		return subbedByteArray;
	}
	
	
	public byte invSubBytes(byte inputByte) {
		...
		//System.out.println("Column#: "+column);
		 this.substitutedString=invSbox(row,column);
	...
	public String invSbox(int x,int y) {
		String sboxReturn;
		String[][] sboxTable={
